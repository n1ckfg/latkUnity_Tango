...
public class ARGUIController : MonoBehaviour, ITangoLifecycle, ITangoDepth
{
	...
	public BrushInputTango brushInputTango;
    ...
    */
    ...
    private void _UpdateLocationMarker()
    {
        if (Input.touchCount == 1)
        {
            ...
            else
            {
				...
				if (showTouchEffect && brushInputTango.rotoEnabled) {
        ...
        if (Input.touchCount == 2 && showDebug)
        {
    ...
    private IEnumerator _WaitForDepthAndFindPlane(Vector2 touchPosition)
    {
		...
		if (snapTarget && brushInputTango.rotoEnabled) {
			depthTarget.transform.position = planeCenter;
			depthTarget.transform.rotation = Quaternion.LookRotation(forward, up);
		}
